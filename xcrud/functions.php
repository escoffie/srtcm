<?php
// Activar o desactivar acceso a sucursal
function publish_action($xcrud)
{
    if ($xcrud->get('primary'))
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE empresa_sucursal_proveedor_esp SET `estatus_esp` = b\'1\' WHERE id_esp = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}
function unpublish_action($xcrud)
{
    ...
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE empresa_sucursal_proveedor_esp SET `estatus_esp` = b\'0\' WHERE id_esp = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}

// Switchea nivel de acceso entre 1 (básico) y 2 (premium)
function basico_action($xcrud)
{
    if ($xcrud->get('primary'))
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE empresa_sucursal_proveedor_esp SET `id_niv` = n\'1\' WHERE id_esp = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}
function premium_action($xcrud)
{
    if ($xcrud->get('primary'))
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE empresa_sucursal_proveedor_esp SET `id_niv` = n\'2\' WHERE id_esp = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}

// Inserta en empresa_sucursal_proveedor_esp un registro por cada proveedor, por cada sucursal, cuando estas no existen
function generaSucursales($postdata, $primary, $xcrud)
{
    if ($postdata->get('codigo_sucursal_suc'))
    {
        $db = Xcrud_db::get_instance();
        $query = 	'INSERT IGNORE INTO surticom_crm.empresa_sucursal_proveedor_esp
					(
						codigo_sucursal_suc,
						codigo_pro
					)
					SELECT a.codigo_sucursal_suc, b.codigo_pro
					FROM  sucursales_suc AS a
					JOIN proveedores_pro AS b
					ORDER BY codigo_pro';
        $db->query($query);
    }
}
